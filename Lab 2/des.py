...
class des():
    ...
    def substitute(self, d_e):  # Substitute bytes using SBOX
        ...
        result = []
        round = 0
        for i in subblocks: # for each subblock
            bin = self.compute_s_box(i, round) # s-box for each subblock
            for j in bin:
                result.append(int(j)) # adds subblock return value into result list
            round += 1
        return result
    ...
    def compute_s_box(self, block, round):
        ...
        row = [int(block[0]), int(block[5])]  # row number
        column = [int(block[1]), int(block[2]), int(block[3]), int(block[4])]

        #  convert to decimal
        row = row[1] * 1 + row[0] * 2
        column = column[3] * 1 + column[2] * 2 + column[1] * 4 + column[0] * 8

        bin = S_BOX[round][row][column]
        bin = binvalue(bin, 4)
        return bin
